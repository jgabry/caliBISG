...
according to caliBISG and traditional BISG. The function has five arguments:
...
- `fips`: a vector of FIPS codes as an alternative to county names
...
most_probable_race(name = "smith", state = "wa", fips = "033")
```
...
the probability of each race. It takes the same arguments as
`most_probable_race()`. The returned object is a standard data frame, however it
has a subclass `"compare_calibisg"` and a custom print method that prints the
results as tables comparing caliBISG and BISG.
