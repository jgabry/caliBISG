...
the `race` column will contain an `NA` and a warning will be thrown. 
...
columns prefixed with `bisg_` and the `bisg_` and `rake_` columns are
interleaved, i.e., ordered to place the races next to each other for easier
comparison.
