...
according to the raking based approach to BISG from Greengard and Gelman (2024).
...
## Compare raking-based BISG to traditional BISG and "improved" BISG

The `compare_race_probabilities()` function allows the user to compare the
raking-based BISG probabilities from Greengard and Gelman (2024) to the
traditional BISG probabilities as well as to the "improved" BISG probabilities.
Here, "improved BISG" refers to traditional BISG but including an adjustment to
those predictions that accounts for the fact that we're making predictions on
registered voters, not the general population. It uses the registered voter
status x race distribution and an application of Bayes rule. Greengard and
Gelman (2024) considers the raking-based approach to be an improvement over both
traditional and "improved" BISG.
...
columns prefixed with `bisg_` (traditional) and `voter_bisg` (improved). The
`bisg_`, `voter_bisg_` and `rake_` columns are also interleaved, i.e., ordered
